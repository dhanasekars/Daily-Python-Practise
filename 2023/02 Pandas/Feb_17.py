...
# Find the median price per two-bedroom listing for each neighborhood.
# Return the three highest neighborhoods with their price from greatest to least.
...
def price_stats(df):
    ...
    """
    # my solution
    # return df[df.bedrooms == 2].groupby('neighborhood').price.median().sort_values(ascending=False).head(3)

    # copilot's solution
    return df[df.bedrooms == 2].groupby('neighborhood').price.median().nlargest(3)