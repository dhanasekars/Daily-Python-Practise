...
# Find the absolute difference the highest and lowest temperature each day for each city.
# Return the date where this difference is greatest for each city.

def max_temp_diff(df):
    """
    :param df:
    :return:
    """
    return (df.resample('D').max() - df.resample('D').min()).idxmax()