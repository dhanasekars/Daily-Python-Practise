...
def temp_count(df):
    """
    Count the number of times each city has the lowest temperature each hour.
    Return a Series sorted from greatest to least
    :param df:
    :return:
    """
    df_new = df.loc[(df.index.month == 7) & (df.index.weekday == 4)]
    return df_new.resample('D').mean().dropna().mean()
